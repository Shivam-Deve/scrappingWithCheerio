...
function scrap(html) {
    ...
    for (let i = 1; i <= 3; i++) {
        let href = $(`#js-pjax-container > div.container-lg.p-responsive.mt-6 > ul > li:nth-child(${i}) > div > a`).attr("href");
...
function scrapAgain(html, topicPath) {
    ...
    for (let i = 1; i <= 8; i++) {
        ...
        //let repos0 = $(aTag[0]).attr("href")

        // if (repos == undefined) {
        //     toBeWritten.push(repos0);
        // } else {
        //     toBeWritten.push(repos)
        // }
        // objectToBeWritten = { ...objectToBeWritten, ...toBeWritten }
    }

    // fs.appendFileSync("links.json", JSON.stringify(objectToBeWritten))
}
...
function scrapIssues(html, repoPath) {
    ...
    let issueArr = $("#repo-content-pjax-container > div > div.Box.mt-3.Box--responsive.hx_Box--firstRowRounded0 > div:nth-child(2) > div div>a.Link--primary")
